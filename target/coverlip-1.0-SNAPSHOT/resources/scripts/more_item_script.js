...
$(document).ready(function () {
    ...
    function loadMoreVideo() {
        ...
        $(window).scroll(function (tt) {
            ...
            if (height < 100 + $('footer').height() && status) {
                $.ajax({
                    type: 'get',
                    url: urlServer,
                    ...
                    success: function (data) {
                        if (data !== null && data.length > 0) {
                            currentItemId = data[data.length - 1].video.id;
                            var string = '';
        ...
        $(window).scroll(function (tt) {
            ...
            if (height < 100 + $('footer').height() && status) {
                $.ajax({
                    type: 'get',
                    url: urlServer,
                    data: {currentItemId: currentPlaylistId, limit: limit, accountId: accountId, type: type},
                    beforeSend: function () {
                        status = false;
                    },
                    success: function (data) {
                        if (data !== null && data.length >0) {
                            currentPlaylistId = data[data.length - 1].id;
                            var string = '';
    ...
    function onePlaylist(itemPlaylist) {
        var string = '';
        string += '<div class="card_playlist">' +
    ...
    function oneIdolFollowing(item){
        return '<div class="col-md-4 one_member text-center">'+
            '<a href="">'+
            '<div class="avatar">'+
